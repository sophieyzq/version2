...
public class Network extends Thread {
	...
	public static boolean transferOut(Transactions outPacket) {
		/*
		 * try { outGoingLocker.acquire(); } catch (InterruptedException e) { // TODO
		 * Auto-generated catch block e.printStackTrace(); }
		 */
		try {
			transferOutLocker.acquire();
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static boolean transferIn(Transactions inPacket) {
		...
		//if (!Network.inBufferStatus.equalsIgnoreCase("empty")) {
			...
			if (getoutputIndexServer() == getinputIndexClient()) {
				...
				// If the buffer is empty, then do not release the transfer in locker
	...
	public void run() {
		...
		while (true) {
			...
			if (clientConnectionStatus.equalsIgnoreCase("disconnected")
					&& serverConnectionStatus.equalsIgnoreCase("disconnected")) {
